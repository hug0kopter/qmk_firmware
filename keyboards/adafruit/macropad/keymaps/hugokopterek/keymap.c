...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    case PREV_TAB:
        if (record->event.pressed) {
            // when keycode PREV_TAB is pressed
        } else {
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [0] = LAYOUT(
      ...
      LGUI(S(KC_TAB)),LGUI(KC_TAB), LCTL(KC_W),
      SEND_STRING("https://www.notion.so/"), KC_UP, LCTL(KC_N),
      SEND_STRING("https://moodle.ucl.ac.uk/"), KC_DOWN, LCTL(S(KC_R)) 
  ), /* safari + music */
  [1] = LAYOUT(
                    KC_MUTE,
      KC_1, KC_2, KC_3,
